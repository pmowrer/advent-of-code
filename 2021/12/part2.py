...
def findEnd(connections, start, path = [], seen = {}, smallSeenTwice = ''):
  ...
  for connection in connectionsFromCurrent:
    if connection == 'end':
      ...
      paths += findEnd(connections, connection, newPath, seen.copy(), smallSeenTwice) 
    elif connection != 'start' and smallSeenTwice == '':
      paths += findEnd(connections, connection, newPath, seen.copy(), connection) 
